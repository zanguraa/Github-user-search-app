...
function UserInfo({ blackTheme, search, result, setResult }) {
  ...
  const createdAt = userData?.created_at?.split("T")[0].split("-");
  ...
  return (
    <UserInfoContainer blackTheme={blackTheme}>
      <HeaderGithub>
        <ProfileImage
          src={userData.avatar_url}
          alt="github-user-image"
        ></ProfileImage>
        <UserNameContainer blackTheme={blackTheme}>
        <UserName>
          <h4>{userData.name}</h4>
      ...
      <Bio blackTheme={blackTheme}>{userData.bio}</Bio>
      <Followers blackTheme={blackTheme}>
        <div>
          <h4>Repos</h4>
      ...
      <div>
        ...
        <div>
          <svg height="20" width="20" xmlns="http://www.w3.org/2000/svg">
            <g fill={`${blackTheme ? "#4B6A9B" : "#FFFFFF"}`}>
...
const UserNameContainer = styled.div`
  ...
  h4 {
    ...
    font-size: 16px;
    line-height: 24px;
    color: ${(props) => (props.blackTheme ? "#2b3442" : "#FFFFFF")};
    @media (min-width: 768px) {
      font-size: 26px;
    }
  }
  ...
  span {
    font-size: 13px;
    line-height: 19px;
    color: ${(props) => (props.blackTheme ? "#697c9a" : "#FFFFFF")};
    @media (min-width: 768px) {
      font-size: 15px;
    }
  }
